...
class CycleGANFaceDetDataset(Dataset):
    def __init__(self, image_real_dir, image_deid_dir, label_dir, transform=None):
        self.image_real_dir = image_real_dir
        self.image_deid_dir = image_deid_dir
        self.label_dir = label_dir
        self.transform = transform

        self.image_real_files = sorted(os.listdir(image_real_dir))
        self.image_deid_files = sorted(os.listdir(image_deid_dir))
        self.txt_files = sorted(os.listdir(label_dir))

    def __len__(self):
        return len(self.image_real_files)

    def __getitem__(self, idx):
        image_real_path = os.path.join(self.image_real_dir, self.image_real_files[idx])
        image_deid_path = os.path.join(self.image_deid_dir, self.image_deid_files[idx])
        label_path = os.path.join(self.label_dir, self.txt_files[idx])

        image_real = Image.open(image_real_path).convert('RGB')
        image_real_width, image_real_height = image_real.size
        if self.transform == None:
            self.transform = transforms.Compose([transforms.ToTensor()])
        image_real = self.transform(image_real)

        image_deid = Image.open(image_deid_path).convert('RGB')
        image_deid_width, image_deid_height = image_real.size
        if self.transform == None:
            self.transform = transforms.Compose([transforms.ToTensor()])
        ...
        return image_real_path, image_deid_path, label_path, image_real, image_deid, boxes_list

    @staticmethod
    def collate_fn(batch):
        image_real_paths = [item[0] for item in batch]
        image_deid_paths = [item[0] for item in batch]
        label_paths = [item[1] for item in batch]
        images_real = [item[2] for item in batch]
        images_deid = [item[2] for item in batch]
        boxes_list = [item[3] for item in batch]

        images_real = torch.stack(images_real, dim=0)
        images_deid = torch.stack(images_deid, dim=0)

        return image_real_paths, image_deid_paths, label_paths, images_real, images_deid, boxes_list